# Standalone high-precision reference generator for scalar Liljegren fixtures.
...
function main()
    return setprecision(BigFloat, 256) do
        ...
        function solar_zenith(time, longitude_deg, latitude_deg)
            ...
            utc_minute = B(60 * hour(time) + minute(time)) +
                         B(second(time)) / b("60") + B(millisecond(time)) / b("60000")
            hour_angle = πb * (utc_minute + eot + b("4") * longitude_deg - b("720")) / b("720")
