...
_legacy_batch_partition(value) = FixedDirectFraction{typeof(value)}(value)

function _legacy_scalar_wbgt(air, dew, wind, ghi, time, longitude, latitude;
                             direct_fraction, kwargs...)
    return HeatStress.liljegren_wbgt(
        air, dew, wind, time, longitude, latitude;
        ghi_w_m2 = ghi,
        partition = _legacy_batch_partition(direct_fraction),
        kwargs...,
    )
end

function _legacy_batch(air, dew, wind, ghi, time, longitude, latitude;
                       direct_fraction, kwargs...)
    return HeatStress.liljegren_wbgt_batch(
        air, dew, wind, time, longitude, latitude;
        ghi_w_m2 = ghi,
        partition = _legacy_batch_partition(direct_fraction),
        kwargs...,
...
function _legacy_batch!(wbgt, wet, globe, air, dew, wind, ghi, time, longitude, latitude;
                        direct_fraction, kwargs...)
    return HeatStress.liljegren_wbgt!(
...
function _legacy_diagnose_batch(air, dew, wind, ghi, time, longitude, latitude;
                                direct_fraction, kwargs...)
    return HeatStress.diagnose_liljegren_batch(
        air, dew, wind, time, longitude, latitude;
        ghi_w_m2 = ghi,
        partition = _legacy_batch_partition(direct_fraction),
        kwargs...,
...
@testset "Liljegren batch model" begin
    ...
    @testset "ordinary, preallocated, and threaded equivalence" begin
        ...
        serial = _legacy_batch(air, dew, wind, radiation, time, 0.0, 0.0; direct_fraction = 0.7)
        ...
        preallocated = _legacy_batch!(wbgt, wet, globe, air, dew, wind, radiation, time, 0.0, 0.0; direct_fraction = 0.7)
        _assert_matches_scalar(preallocated, expected)

        threaded = _legacy_batch(
        ...
        one_row = _legacy_batch(air[1:1], dew[1:1], wind[1:1], radiation[1:1], time[1:1], 0.0, 0.0; direct_fraction = 0.7)
        @test one_row.wbgt_c == serial.wbgt_c[1:1]
        empty = _legacy_batch(Float64[], Float64[], Float64[], Float64[], DateTime[], 0.0, 0.0; direct_fraction = 0.7)
    ...
    @testset "shared typed row execution" begin
        ...
        from_time = HeatStress._liljegren_row_from_time(
            air[1], dew[1], wind[1], time[1], -74.0060, 40.7128,
            1010.0, radiation[1], nothing, nothing,
            FixedDirectFraction(0.7), config, HeatStress._ValueMode(),
        )
        zenith = deg2rad(HeatStress.solar_zenith(time[1], -74.0060, 40.7128))
        irradiance = HeatStress._resolve_irradiance(
            radiation[1], nothing, nothing, FixedDirectFraction(0.7),
            time[1], zenith, config,
        )
        from_zenith = HeatStress._liljegren_row_from_zenith(
            air[1], dew[1], wind[1], irradiance, zenith, 1010.0,
            config, HeatStress._ValueMode(),
        )
        scalar = _legacy_scalar_wbgt(
    ...
    @testset "ordinal AbstractVector indexing and permissive outputs" begin
        ...
        any_result = _legacy_batch!(any_outputs..., air, dew, wind, radiation, time, 0.0, 0.0; direct_fraction = 0.7)
        ...
        real_result = _legacy_batch!(real_outputs..., air, dew, wind, radiation, time, 0.0, 0.0; direct_fraction = 0.7)
    ...
    @testset "pre-mutation input, output, and alias rejection" begin
        air, dew, wind, radiation, time = _batch_inputs()
        wbgt, wet, globe = _sentinel_outputs(4)
        _assert_premutation_failure(() -> _legacy_batch!(wbgt[1:3], wet, globe, air, dew, wind, radiation, time, 0.0, 0.0; direct_fraction = 0.7), wbgt, wet, globe)
        _assert_premutation_failure(() -> _legacy_batch!(Vector{Float64}(undef, 4), wet, globe, air, dew, wind, radiation, time, 0.0, 0.0; direct_fraction = 0.7), wbgt, wet, globe)
        for output_rows in (3, 5, 0)
            wbgt, wet, globe = _sentinel_outputs(output_rows)
            _assert_premutation_failure(
                () -> _legacy_batch!(wbgt, wet, globe, air, dew, wind, radiation, time, 0.0, 0.0; direct_fraction = 0.7),
                wbgt, wet, globe,
            )
        end
        ...
        for (bad_air, bad_dew, bad_wind, bad_radiation, bad_time, longitude, latitude, pressure, fraction) in invalid_cases
            wbgt, wet, globe = _sentinel_outputs(4)
            _assert_premutation_failure(
                () -> _legacy_batch!(wbgt, wet, globe, bad_air, bad_dew, bad_wind, bad_radiation, bad_time, longitude, latitude; pressure_hpa = pressure, direct_fraction = fraction),
                wbgt, wet, globe,
            )
        end

        wbgt, wet, globe = _sentinel_outputs(4)
        _assert_premutation_failure(() -> _legacy_batch!(wbgt, wbgt, globe, air, dew, wind, radiation, time, 0.0, 0.0; direct_fraction = 0.7), wbgt, wet, globe)
        aliased_air = fill!(Vector{Union{Missing,Float64}}(undef, 4), 30.0)
        _, wet, globe = _sentinel_outputs(4)
        wbgt = aliased_air
        _assert_premutation_failure(() -> _legacy_batch!(wbgt, wet, globe, aliased_air, dew, wind, radiation, time, 0.0, 0.0; direct_fraction = 0.7), wbgt, wet, globe)
        ...
        _assert_premutation_failure(() -> _legacy_batch!(overlapping_wbgt, overlapping_wet, globe, air, dew, wind, radiation, time, 0.0, 0.0; direct_fraction = 0.7), overlapping_wbgt, overlapping_wet, globe)
    end
    ...
    @testset "type and missingness equivalence" begin
        ...
        for (inputs, longitude, latitude, pressure, fraction, config, expected_type) in cases
            ...
            batch = _legacy_batch(air, dew, wind, radiation, time, longitude, latitude; pressure_hpa = pressure, direct_fraction = fraction, config)
        ...
        batch = _legacy_batch(
            ...
            pressure_hpa = missing_pressure, direct_fraction = 0.7,
        )
        ...
        @test_throws ArgumentError _legacy_batch(
            air, dew, wind, radiation, time, 0.0, 0.0;
            direct_fraction = missing_fraction,
        )

        all_missing_air = fill(missing, 4)
        all_missing_batch = _legacy_batch(
        ...
        all_missing_pressure_batch = _legacy_batch(
            air, dew, wind, radiation, time, 0.0, 0.0;
        ...
        rejected_and_failed = _legacy_batch(
    ...
    @testset "complete diagnostic equivalence" begin
        ...
        serial = _legacy_diagnose_batch(air, dew, wind, radiation, time, 0.0, 0.0; direct_fraction = 0.7)
        threaded = _legacy_diagnose_batch(air, dew, wind, radiation, time, 0.0, 0.0; direct_fraction = 0.7, threaded = true)
    ...
    @testset "independent scalar fixtures" begin
        ...
        serial = _legacy_batch(fixture_air, fixture_dew, fixture_wind, fixture_radiation, fixture_time, fixture_longitude, fixture_latitude; pressure_hpa = fixture_pressure, direct_fraction = fixture_direct_fraction)
        threaded = _legacy_batch(fixture_air, fixture_dew, fixture_wind, fixture_radiation, fixture_time, fixture_longitude, fixture_latitude; pressure_hpa = fixture_pressure, direct_fraction = fixture_direct_fraction, threaded = true)
