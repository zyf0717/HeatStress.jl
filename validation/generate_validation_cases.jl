...
module ValidationReferenceGenerator
...
function main(args::Vector{String} = ARGS)
    ...
    destination = joinpath(FIXTURES, "liljegren_reference.csv")
    if args == ["--check"]
        reference_matches() || error("$destination differs from deterministic high-precision regeneration")
        return nothing
    end
    open(destination, "w") do io
        write(io, reference_csv())
    end
    return nothing
end
...
abspath(PROGRAM_FILE) == abspath(@__FILE__) && ValidationReferenceGenerator.main()
